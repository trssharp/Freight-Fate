...
| A National Weather Service warning active on the road, spoken with the Service's own product name after it ("Weather alert: High Wind Warning, gusts to 60 miles per hour") | Weather alert | weather warning (as the noun), advisory, watch, bulletin, NWS alert | `sim/real_weather_alerts.rs`, `DrivingState::sync_weather_alerts`, `real_weather_alerts` setting |
