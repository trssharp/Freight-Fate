...
def find_hold(x: np.ndarray) -> tuple[float, float, float]:
    ...
    lo, hi = np.searchsorted(f, (24.0, 120.0))
    ff = f[lo:hi]
    prev = 33.0
    T = []
    R = []
    ...
    for i in range(0, len(x) - win, hop):
        s = xl[i : i + win] * np.hanning(win)
        S = np.abs(np.fft.rfft(s))[lo:hi]
        ...
        if S.max() < 1e-6 or rms < 0.003:
            ...
            prev = 33.0
            continue
        w = S * np.exp(-(((ff - prev) / 14.0) ** 2))
        pk = ff[int(np.argmax(w))]
        prev = 0.6 * prev + 0.4 * pk
        T.append(i / SR)
        R.append(pk * 20)
        L.append(rms)
    R = np.array(R)
...
def main() -> None:
    ...
    print(
        f"  base loop {len(loop) / SR:.2f}s cycle {cyc * 1000:.0f}ms seam {C.seam_check(loop):.2f}"
    )
